...
:
    ...
    qrName_(rhs.qrName_),
    minTemperature_(rhs.minTemperature_),
    maxTemperature_(rhs.maxTemperature_),
...
void Foam::externalWallHeatFluxTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    {
        const scalarField patchInternal(this->patchInternalField());
        const scalarField deltaCoeffs(this->patch().deltaCoeffs());

        forAll(patchInternal, i)
        {
            const scalar fixedValuePart =
                patchInternal[i] + refGrad()[i]/deltaCoeffs[i];

            const scalar patchValue =
                lerp(fixedValuePart, refValue()[i], valueFraction()[i]);

            const scalar limitedValue =
                min(max(patchValue, minTemperature_), maxTemperature_);

            if (limitedValue == patchValue)
            {
                continue;
            }

            if (valueFraction()[i] <= SMALL)
            {
                refGrad()[i] =
                    (limitedValue - patchInternal[i])*deltaCoeffs[i];
            }
            else
            {
                refValue()[i] =
                (
                    limitedValue
                  - (1 - valueFraction()[i])*fixedValuePart
                )/valueFraction()[i];
            }
        }
    }
