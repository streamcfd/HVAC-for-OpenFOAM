    ...
    pp_(pp)
{
    ...
    if (dict.found("wallTransmissivityModel"))
    {
        transmissivity_.reset(wallTransmissivityModel::New(dict, pp).ptr());
    }
...
) const
{
    if (transmissivity_)
    {
        return transmissivity_->t(bandI, dir, T);
    }
...
) const
{
    if (transmissivity_)
    {
        return transmissivity_->t(faceI, bandI, dir, T);
    }
